...
### D1数据库支持
- **高性能存储**: 从KV迁移到D1关系型数据库
- **复杂查询**: 支持复杂的访问记录查询和分析
- **数据完整性**: 更好的数据一致性和事务支持
- **成本优化**: 更低的存储成本，特别适合大量数据

### 企业级风控系统
- **设备指纹识别**: 精确追踪访问设备
- **代理工具检测**: 识别Clash、V2Ray等代理工具
- **国家地理限制**: 限制特定国家/地区访问
- **智能告警**: 异常访问实时Telegram通知
...
为了获得更好的性能和更强大的查询能力，我们提供了从KV存储迁移到D1数据库的方案：

### 迁移优势
- **高性能**: 关系型数据库，查询速度更快
- **复杂查询**: 支持JOIN、GROUP BY等复杂查询
- **数据完整性**: 更好的数据一致性和事务支持
- **成本优化**: 大量数据存储成本更低

### 快速迁移
1. 创建D1数据库：`wrangler d1 create myurls-db`
2. 应用Schema：`wrangler d1 execute myurls-db --file=./schema.sql`
3. 运行迁移：`node migrate-to-d1.js`
4. 更新配置：替换API文件为D1版本

详细迁移指南请参考：[D1数据库迁移指南](docs/d1-migration-guide.md)
